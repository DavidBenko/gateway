...
func TestMesh(t *testing.T) {
	...
	log.Infof("Total Received %d messages in %d ns, %d ns/msg, %d msgs/sec", totalRcvd, rcvdSince, int(float64(rcvdSince)/float64(totalRcvd)), int(float64(totalRcvd)/(float64(rcvdSince)/float64(time.Second))))
}
	...
	runClientTest(t, cid, wg, func(svc *service.Client) {
		...
		svc.Subscribe(sub,
			...
			func(msg *message.PublishMessage) error {
				...
				received++
				//log.Debugf("(surgemq%d) messages received=%d", cid, received)
				since = time.Since(now).Nanoseconds()
		...
		case <-time.After(time.Second * time.Duration(publishers)):
			log.Infof("(surgemq%d) Timed out waiting for subscribe response", cid)
			return
		}
		...
		go func() {
			...
			log.Debugf("(surgemq%d) Sent %d messages in %d ns, %d ns/msg, %d msgs/sec", cid, sent, since, int(float64(since)/float64(cnt)), int(float64(sent)/(float64(since)/float64(time.Second))))
		}()

		select {
		case <-done2:
		case <-time.Tick(time.Second * time.Duration(nap*publishers)):
			log.Errorf("Timed out waiting for messages to be received.")
		}
		...
		log.Debugf("(surgemq%d) Received %d messages in %d ns, %d ns/msg, %d msgs/sec", cid, received, since, int(float64(since)/float64(cnt)), int(float64(received)/(float64(since)/float64(time.Second))))
	})
}
