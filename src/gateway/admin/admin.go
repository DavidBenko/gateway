...
func Setup(router *mux.Router, db *sql.DB, configuration config.Configuration, c *core.Core) {
	...
	scratchPadController := &MetaRemoteEndpointEnvironmentDatumScratchPadsController{RemoteEndpointEnvironmentDatumScratchPadsController{base}, c}
	RouteRemoteEndpointEnvironmentDatumScratchPads(scratchPadController, "/apis/{apiID}/remote_endpoints/{endpointID}/environment_data/{environmentDataID}/scratch_pads", authAdmin, db, conf)
