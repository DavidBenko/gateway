...
func performInTransaction(db *apsql.DB, method transactional) error {
	...
	if methodErr != nil {
		err = tx.Rollback()
		if err != nil {
			logger.Printf("%s Error rolling back transaction!", config.System)
		}
		return methodErr
	}
...
func TransactionWrappedHandler(db *apsql.DB, handler TransactionAwareHandler) aphttp.ErrorReturningHandler {
	return func(w http.ResponseWriter, r *http.Request) aphttp.Error {
		tx, err := db.Begin()
		if err != nil {
			logger.Printf("%s Error beginning transaction: %v", config.System, err)
			return aphttp.DefaultServerError()
		}
		handlerError := handler(w, r, tx)
		if handlerError != nil {
			if err = tx.Rollback(); err != nil {
				logger.Printf("%s Error rolling back transaction: %v", config.System, err)
			}
		} else {
			if err = tx.Commit(); err != nil {
				logger.Printf("%s Error committing transaction: %v", config.System, err)
			}
		}
		return handlerError
	}
