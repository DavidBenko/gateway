...
func (l *soapNotificationListener) Notify(n *apsql.Notification) {
	...
	case apsql.Update, apsql.Insert:
		...
		if err != nil {
			logger.Printf("%s Error caching jarfile for api %d: %v", config.System, n.APIID, err)
		}
	case apsql.Delete:
		...
		if remoteEndpointID, ok = n.Messages[0].(int64); !ok {
			...
			if !ok {
				logger.Printf("%s Error deleting jarfile for api %d: %v", config.System, n.APIID, "deletion message did not come in expected format")
				return
			}
		}
		...
		if err != nil {
			logger.Printf("%s Error deleting jarfile for api %d: %v", config.System, n.APIID, err)
		}
	}
}
...
func DeleteJarFile(soapRemoteEndpointID int64) error {
	logger.Printf("Received a request to delete jar file for soapRemoteEndpointID %d", soapRemoteEndpointID)
...
func processWsdl(db *apsql.DB, e *SoapRemoteEndpoint, tag string) {
	...
	if err != nil {
		logger.Printf("%s Unable to update status to %v.  Processing will continue anyway.", config.System, RemoteEndpointStatusProcessing)
	}
	...
	if err != nil {
		logger.Printf("%s WSDL could not be processed due to error encountered:  %v", config.System, err)
	...
	if err != nil {
		logger.Printf("%s Unable to update status of RemoteSoapEndpoint due to error: %v", config.System, err)
	}
}

func ingestWsdl(tx *apsql.Tx, e *SoapRemoteEndpoint) error {
	logger.Printf("%s Starting wsdl ingestion", "[debug]")
	...
	if err != nil {
		logger.Printf("%s Tried to invoke wsimport: %v", "[debug]", err)
		return err
	}
	...
	if err != nil {
		logger.Printf("%s Couldn't read bytes! %v", "[debug]", err)
		return err
	}
	...
	if err != nil {
		logger.Printf("%s Unable to notify of update: %v", "[debug]", err)
		return err
	}
	...
	logger.Printf("%s Finished wsdl ingestion", "[debug]")
